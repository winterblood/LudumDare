...
@System.NonSerialized
var inputJumpDB : boolean = false;

// Is the fire button held down? We use this interface instead of checking
// for the fire button directly so this script can also be used by AIs.
@System.NonSerialized
var inputFire : boolean = false;
@System.NonSerialized
var inputFireDB : boolean = false;

private var tower : GameObject;
private var towerMap : TowerGenerator;
...
class CharacterMotorGrabbing
{
	var enabled : boolean = true;

	var facingTolerance : float = 0.85f;

	var velocityScale : float = 0.5f;

	@System.NonSerialized
	var grabTimer : float = 0.0f;

	@System.NonSerialized
	var grabBlockPos : Vector3;

	@System.NonSerialized
	var grabFacing : Vector3;

	@System.NonSerialized
	var grabStartPos : Vector3;
}

var grabbing : CharacterMotorGrabbing = CharacterMotorGrabbing();
...
function Awake ()
{
	...
	tower = GameObject.Find("TowerRoot");
	towerMap = tower.GetComponent(TowerGenerator);
...
private function UpdateFunction ()
{
	...
	if (GrabInProgress())
		velocity = ProcessGrabbingManeuver(velocity);
...
function OnControllerColliderHit (hit : ControllerColliderHit)
{
	...
	if (body)
	{
		if (body.isKinematic)	// Still attached? Check for crumble
		{
			if (hit.normal.y > 0.9f && !grounded)
			{
				//Debug.Log("Landed on dynamic cube!");
				//Debug.Log(hit.gameObject.name);
				hit.gameObject.SendMessage("OnStandOn", SendMessageOptions.DontRequireReceiver);
			}
		}
		else	// Don't attempt to mantle onto dynamic rigids
		{
			return;
		}
    }

	var block_pos : Vector3 = hit.collider.transform.position;
	if (block_pos.y > 0.9F)
	{
		//
		// Must be a cube (not the floor). Check for horizontal collisions that are nearly opposite the cube face normal
		//
		if (hit.moveDirection.y*hit.moveDirection.y < 0.1F && Vector3.Dot( hit.normal, hit.moveDirection ) < -0.85F)
		{
			if (Vector3.Dot(transform.forward, hit.moveDirection) > 0.85f)	// Player must be facing block
			{
				var inputMoveDir : Vector3 = inputMoveDirection;
				inputMoveDir.y = 0.0f;
				if (Vector3.Dot(inputMoveDirection, hit.moveDirection) > 0.85f)	// Player must be pushing against block
				{
					if ((transform.position.y < block_pos.y + towerMap.GetCubeSize() * 0.5f) &&
						(transform.position.y > block_pos.y - towerMap.GetCubeSize() * 0.25f))
					{
						if (towerMap.IsCubeAtPositionClimbable( block_pos ))
						{
							BeginGrabbingManeuver( hit );
						}
					}
				}
			}
...
private function BeginGrabbingManeuver (hit : ControllerColliderHit)
{
	if (!grabbing.enabled || GrabInProgress())
		return;

	Debug.Log("BeginGrabbingManeuver");
	grabbing.grabBlockPos = hit.collider.transform.position;
	grabbing.grabFacing = -hit.normal;
	grabbing.grabStartPos = transform.position;
	grabbing.grabTimer = 0.00001F;
	SetControllable(false);
}

private function ProcessGrabbingManeuver ( velocity : Vector3 ) : Vector3
{
	if (!grabbing.enabled || !GrabInProgress())
		return;

	var targetPos : Vector3 = grabbing.grabBlockPos;
	targetPos.y += towerMap.GetCubeSize() * 0.5f + 1.0f + 0.3f;	// Add half cube size PLUS half of player capsule PLUS tolerance

	// Two phase process - phase 1 = lift up, phase 2 = forwards
	var phase1 : float = grabbing.grabTimer * 2.0f;
	var phase2 : float = (grabbing.grabTimer - 0.5f) * 2.0f;


	var travel : Vector3;
	var travelpos : Vector3;
	if (phase1 < 1.0f)
	{
		travel 		= targetPos - grabbing.grabStartPos;
		travel.x 	= 0.0f;
		travel.z 	= 0.0f;
		travelpos 	= grabbing.grabStartPos + travel * phase1;	// Travel straight up
	}
	else
	{
		travel		= targetPos - grabbing.grabStartPos;
		travel.y 	= 0.0f;
		travelpos 	= grabbing.grabStartPos + travel * phase2;	// Travel forwards
		travelpos.y = targetPos.y;
	}
	var vel : Vector3 = travelpos - transform.position;
	velocity = vel * grabbing.velocityScale;

	grabbing.grabTimer += Time.deltaTime;
	if (grabbing.grabTimer > 1.0F)
	{
		Debug.Log("EndGrabbingManeuver");
		SetControllable(true);
		grabbing.grabTimer = 0.0F;
...
private function GrabInProgress () : boolean
{
	return grabbing.grabTimer > 0.0F;
}
